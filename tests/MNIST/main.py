...
from models.cnn_models_v2 import build_detector, build_discriminator, CAENet2D
...
X_train_rs = np.zeros((len(X_train), 48, 48))
X_test_rs = np.zeros((len(X_test), 48, 48))

for i in range(len(X_train_rs)):
    X_train_rs[i] = cv2.resize(X_train[i], (48, 48))
for i in range(len(X_test_rs)):
    X_test_rs[i] = cv2.resize(X_test[i], (48, 48))

X_train, X_test = X_train_rs, X_test_rs
input_shape = (48, 48, 1)
