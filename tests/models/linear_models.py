...
class TabNoise(Layer):
	...
	def __init__(self, output_dim, regularizer=tf.keras.regularizers.L1(1.), initializer=initializers.Zeros(), **kwargs):
	...
	def call(self, X):
	   ...
	   # return X - K.sigmoid(self.D_) - .5

	def compute_output_shape(self):
	   return self.output_dim


def build_detector(input_dim, regularizer, initializer, type_='mask'):
	model = Sequential()
	model.add(tf.keras.Input(shape=input_dim))
	model.add(TabNoise(input_dim, regularizer=regularizer, initializer=initializer))
	# if type_ == 'mask':
