...
class loc_model(object):
    ...
    """
    def __init__(self, input_shape, discriminator, detector_backend, detector=None, tau_range = np.arange(1,10), target_r_square='auto', r_metric='loss'):
        # self.labels = labels
    ...
    def build_detector(self, tau=10., max_value=1.):
        ...
        """
        self.detector_backend.layers[-1].activation = tf.keras.layers.Activation('linear')
    ...
    def fit(self, X_train, y_train, fit_params, demo_ind=None, X_test=None, y_test=None, optimizer=SGD(learning_rate=.0005)):
        """
        ...
        if self.target_r_square == 'auto':
            ...
            if self.r_metric == 'loss':
                self.target_r_square = 1. - loss_base / loss_random
            else:
                self.target_r_square = 1. - acc_random / acc_base
        ## fitting the models
        reach_target = 0
    ...
    def R_square(self, X, y):
        ...
        """
        print('#'*50)
        print('compute the R2 for the fitted detector.')
        print('#'*50)
