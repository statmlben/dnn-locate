...
if wandb_flag:
    ...
    WANDB_NOTEBOOK_NAME='ECG-DF-detector: with all classes'
...
demo_ind = np.array([np.random.choice(np.where(y[:,k] == 1)[0]) for i in range(n_demo) for k in [0,1,2,3,4]])
print('show instances with indices: %s' %demo_ind)
X_demo = X[demo_ind]
y_demo = y[demo_ind]

detetor_folders = ['detector_r2_10', 'detector_r2_50', 'detector_r2_60',
                   'detector_r2_70', 'detector_r2_75', 'detector_r2_80',
                   'detector_r2_86', 'detector_r2_88']
...
# fig, axs = plt.subplots(5,2,sharex=True, sharey=True, gridspec_kw={'hspace': 0.1, 'wspace': 0.05})
for i in range(len(X_demo)):
    ...
    for j in range(len(detetor_folders)):
        ...
        # axs[y_tmp,r].imshow(X_hl_tmp[np.newaxis,:], cmap=cmap_tmp, aspect='auto', alpha=0.7, 
        #                                 extent = (0, 187, 0, 1))
        # axs[y_tmp,r].plot(timepoint, X_tmp, linewidth=2, alpha=.7, color=color_tmp1, label=class_dict[y_tmp])
        # axs[y_tmp,r].title.set_text('Extracted ECG: (sample %s, class %s)' %(r, class_dict[y_tmp]))
        # axs[y_tmp,r].axis('off')
        ...
        plt.imshow(X_hl_tmp[np.newaxis,:], cmap=cmap_tmp, aspect='auto', alpha=0.7, 
                                            extent = (0, 187, 0, 1))
        plt.clim(0, 1)
        plt.colorbar()
        plt.plot(timepoint, X_tmp, linewidth=2.5, alpha=.7, color=color_tmp1, 
            label='Extracted ECG Beat')
        ...
        # plt.title('Extracted ECG: (sample %s, class %s)' %(r, class_dict[y_tmp]))
        plt.legend(loc='best')
        # axs[i].tight_layout()
        if wandb_flag:
